...
class MainMenuState( State ):
    ...
    def PlayBGMusic( self ):
        pygame.mixer.music.load( os.path.join( os.path.join( 'data' , 'music' ) , "ibnal-noor.mp3" ))
    ...
    def ExecuteSettingsMenu( self ):
        ...
        for button in range( 3 ):
            if( self.MenuButtons[button+4].IsMouseInside() ):
                if ( pygame.mouse.get_pressed()[0] and self.MouseReleased ):
        ...
        if( self.MenuButtons[10].IsMouseInside() ):
                if ( pygame.mouse.get_pressed()[0] and self.MouseReleased ):
